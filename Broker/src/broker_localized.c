...
void devolver_localized_pokemon(int socket_cliente,
...
void almacenar_en_cola_localized_pokemon(t_localized* mensaje) {
	...
	t_localized* elemento_agregado = list_get(localized_pokemon->mensajes,
			size - 1);
	log_trace(logger,
			"Se agrego a la cola LOCALIZED_POKEMON el mensaje con id: %i.",
...
void enviar_a_todos_los_subs_localized_pokemon(t_localized* mensaje) {
	...
	list_iterate(localized_pokemon->subscriptores,
...
void enviar_localized_pokemon_a_suscriptor(t_suscriptor_queue* suscriptor,
		t_localized* mensaje) {

	log_trace(logger,
			"Se va a enviar mensaje LOCALIZED_POKEMON id: %i a sub: %i.",
