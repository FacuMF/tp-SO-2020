...
void manejar_mensaje_caught(t_conexion_buffer *combo) {
	...
	log_trace(logger, "Se devolvio el mensaje CAUGHT_POKEMON con id asignado.");
...
void devolver_caught_pokemon(int socket_cliente,
...
void almacenar_en_cola_caught_pokemon(t_caught_pokemon* mensaje) {
	...
	t_caught_pokemon* elemento_agregado = list_get(caught_pokemon->mensajes,
			size - 1);
	log_trace(logger,
			"Se agrego a la cola CAUGHT_POKEMON el mensaje con id: %i.",
...
void enviar_a_todos_los_subs_caught_pokemon(t_caught_pokemon* mensaje) {
	...
	list_iterate(caught_pokemon->subscriptores,
...
void enviar_caught_pokemon_a_suscriptor(t_suscriptor_queue* suscriptor,
